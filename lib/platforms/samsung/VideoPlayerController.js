(function(exports){
  ...
  let VideoPlayerController = function(){
    ...
    this.init = options => {
      ...
      zypeApi.getPlayer(this.videoId, auth)
      .then(
        resp => {
    ...
    this.setVideoPlaytime = time => {
      ...
      StorageManager.playbackTimes.setVideoTime(this.videoId, time);
    };
    this.deleteVideoPlaytime = () => StorageManager.playbackTimes.deleteVideoTime(this.videoId);
        ...
        webapis.avplay.prepareAsync(
          function(){
            if (!_this.stillInApp) { // user left the app during load
              _this.view.trigger("updateTime", _this.playbackTime / 1000);
