(function(exports){
	...
	let VideoPlayerController = function(){
		 ...
		 */
		this.shouldPlayAds = () => {
			return (this.adTimings && this.adTimings.length > 0) ? true : false;
		};

		this.playerAtAdTiming = () => {
			let adTiming = this.adTimings[ this.adTimings.length - 1 ];
			let timeDiff = Math.abs(webapis.avplay.getCurrentTime() - adTiming.offset);
		...
		this.prepareAVPlayer = () => {
			...
			let ads = this.playerInfo.body.advertising;

			try {
				webapis.avplay.open(source.url);

				if (ads && ads.schedule) {
					for (let i = ads.schedule.length - 1; i > -1; i--) {
						this.adTimings.push(ads.schedule[i]);
					}
				}
				...
				webapis.avplay.setListener({
					oncurrentplaytime: function(currentTime){
						...
						if (_this.shouldPlayAds() && _this.playerAtAdTiming()) {
							let adTiming = _this.adTimings.pop();

							_this.playerReady = false;
							_this.pauseVideo();
							AdsService.requestAd(adTiming.tag, _this.pauseVideo, _this.playVideo);
						}
					},
					...
					function(){
						if (!_this.playerClosed) {
							...
							if (_this.shouldPlayAds() && _this.playerAtAdTiming()) {
								...
								_this.playerReady = false;
								_this.pauseVideo();
								AdsService.requestAd(adTiming.tag, _this.pauseVideo, _this.handlePlay);
							}
							else {
								hideSpinner();
								webapis.avplay.play();
		...
		this.handleButtonPress = buttonPress => {
			switch (buttonPress) {
						...
						try {
							...
							let successCallback = () => {
								if (this.adTimings) {
									...
									while (
										this.adTimings.length > 0 &&
										this.adTimings[this.adTimings.length - 1].offset < currentTime
									) {
										this.adTimings.pop();
									}
								}
				...
				case TvKeys.PLAY:
					if (this.playerReady) this.handlePlay();
					break;
				case TvKeys.PAUSE:
					if (this.playerReady) this.handlePause();
					break;
				case TvKeys.STOP:
