...
(function(){
	...
	var ZypeJSBase = function(configs){
		...
		this.getVideosByCategory = (categoryTitle, categoryValue, params) => {
			let url = zypeUrl("api", ZypeEndpoints.api.getVideos());
			let categoryKey = "category[" + categoryTitle + "]";
		...
		this.getVideosByZObject = (zObjectId, params) => {
			let url = zypeUrl("api", ZypeEndpoints.api.getVideos());
			params.app_key = this.appKey;
			params.zobject_id = zObjectId;
			return httpRequest("GET", url, params);
		};
		...
		this.refreshAccessToken = function(refresh_token){
			var url = zypeUrl("api", ZypeEndpoints.api.refreshAccessToken());
			var params = {
				client_id: this.clientId,
				client_secret: this.clientSecret,
